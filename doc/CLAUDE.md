...
- **Neutral working directory**: agy indexes its cwd, so both the daemon and the one-shot fallback run from an empty scratch dir under the runtime dir. Measured from inside this repo: 6.4s median before, 5.1-5.2s after, with far less variance. Input tokens are unaffected — this is indexing wall-clock, not prompt size
...
- **Measured dead ends** (don't re-try these):
  - `--conversation <id>` to resume a prior conversation on the one-shot path *hurts*: over three successive turns the model time went 5.9s → 11.0s → 16.1s and input grew 11.7k → 13.9k → 28.4k. Resuming re-sends the whole history. In-process reuse (the daemon) is fine because the process keeps its state warm; the resume path is not
  - agy's ~13.9k-token preamble sometimes bills as ~5.8k input + ~8.1k `cache_read_tokens`. That is a server-side prefix cache firing at random on *fresh* conversations — it is not evidence that anything we did shrank the prompt
  - `--disable-slash-commands` changes neither input tokens (13.9k either way) nor wall clock beyond noise
